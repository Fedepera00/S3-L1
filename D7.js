...
function concatStrings(str1, str2) {
  const result = (str1[0] + str1[1] + str2[str2.length - 3] + str2[str2.length - 2] + str2[str2.length - 1]).toUpperCase();
  console.log(result);
}

concatStrings("Cavallo", "Giraffa");
...
console.log(incrementArray([1, 2, 3], 8));
/* ESERCIZIO 7 (map)
...
function findOldestMovie(array) {
  let findOldestMovie = 0;
  array.forEach((movie) => console.log(movie.Year));
}

findOldestMovie(movies);
...
*/
function getMovies2000(movies) {
  return movies.filter((movie) => parseInt(movie.Year) >= 2000);
}

console.log(getMovies2000(movies));
...
function findMovieByImdbID(movies, imdbID) {
  return movies.find((movie) => movie.imdbID === imdbID);
}

console.log(findMovieByImdbID(movies, "tt4154796"));

/* ESERCIZIO 15 (findIndex)
  Scrivi una funzione per ottenere dall'array fornito l'indice del primo film uscito nell'anno fornito come parametro.
*/
function findFirstMovie(movies, year) {
